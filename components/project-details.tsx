...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ project, posts }) => {
    const [participants, setParticipants] = useState<string[]>(project.participants || []);
    ...
    const handleAddParticipant = async (newParticipant: string) => {
        ...
        const result = await updateParticipants(project._id.toString(), newParticipant);
        if (result) {
                ...
                <div className="p-5">
                    <div className="flex flex-col">
                        <div className="flex flex-row gap-2 justify-between items-center">
                            ...
                            {session?.user?.email && (
                                <div className="ml-auto">
                                    <Button
                                        onClick={() => handleAddParticipant(session?.user?.email ?? "")}
                        ...
                        <div className="flex flex-col pt-5">
                            ...
                            <div className="text-s text-zinc-600">{project.description}</div>
                        </div>
                        <div className="flex flex-col pt-5">
                        ...
                        <div className="flex flex-col pt-5">
                            <div className="text-xl font-medium text-black">Themes</div>
                            <div className="text-s text-zinc-600">{project.themes?.join(", ")}</div>
                        </div>
                        <div className="flex flex-col pt-5">
                            <div className="text-xl font-medium text-black">Posts</div>
