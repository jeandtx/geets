			...
			<div className="max-w-4xl mx-auto">
				<div className="bg-white shadow-lg rounded-lg p-8 space-y-6">
					<div className="flex flex-col items-center">
						<h1 className="text-3xl font-extrabold text-gray-900">{user?.email}</h1>
						{user ? (
							<table className="min-w-full divide-y divide-gray-200 mt-6">
								<thead>
									<tr>
										<th className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Field</th>
								...
								<tbody className="bg-white divide-y divide-gray-200">
									{[
										...
										{ label: "Account Creation", value: user.createdAt },
									].map(({ label, value }) => (
										<tr key={label}>
							...
							<div className="mt-6 text-center text-gray-500">User not found :(</div>
						)}
					</div>
					<div className="space-y-8">
										...
										<div key={post._id} className="bg-white rounded-lg shadow-lg overflow-hidden">
											<div className="p-6">
												<div className="text-sm text-gray-600 mb-2">
													{post.project?.title ? post.project.title : "Nom de projet"} • {post.project?._id || "ID de projet"}
												</div>
						...
						<section>
							...
							{Array.isArray(userProjects) && userProjects.length > 0 ? (
								...
								<div className="mt-6 text-center text-gray-500">No projects found</div>
							)}
						</section>
						<section>
							<h2 className="text-2xl font-semibold text-gray-900">Projects {decodeEmail} is participating in</h2>
							{participatingProjects.length > 0 ? (
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
									{participatingProjects.map(async (project: Project) => {
										const author = project.participants.find(
											(participant) => participant.role === "author"
										);
										const authorInfo = await getUser(author?.name || "");
										return (
											<div key={project._id.toString()} className="bg-white rounded-lg shadow-lg overflow-hidden">
												<div className="p-6">
													<Link href={`/${author?.name}/${project._id}`} className="text-lg font-bold text-blue-600 hover:underline">
														{project.title}
													</Link>
													<p className="text-sm text-gray-600 mt-2">{project.description}</p>
													<p className="text-sm text-gray-600 mt-2">Author: {authorInfo ? authorInfo.email : "Unknown"}</p>
												</div>
											</div>
										);
									})}
								...
								<div className="mt-6 text-center text-gray-500">No participating projects found</div>
							)}
						</section>
					</div>
					<SignOut />
